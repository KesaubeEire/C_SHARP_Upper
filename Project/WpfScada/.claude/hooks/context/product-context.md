...
**WpfScada** 是基于 Wpf.Ui 组件库（NuGet 4.3.0）的 WPF 上位机测试项目，用于西门子 PLC (S7) 的调试与监控。基于 WpfScada 裁剪改造，保留其现代化 UI 框架，替换业务逻辑为 S7 通信 + Modbus。
...
- **私有字段**: `_camelCase` 前缀
- **namespace 沿用 `WpfScada`**（原始项目命名空间，后续可改）
