...
namespace WpfScada.Views.Windows;
...
public partial class MainWindow : IWindow
{
    ...
    )
    {
        ...
        // 恢复上次保存的主题（如果配置有值）
        if (Enum.TryParse<Wpf.Ui.Appearance.ApplicationTheme>(config.ThemeMode, out var savedTheme))
            Wpf.Ui.Appearance.ApplicationThemeManager.Apply(savedTheme);
    ...
    private void OnThemeToggle(object sender, RoutedEventArgs e)
    {
        ...
        config.ThemeMode = newTheme == Wpf.Ui.Appearance.ApplicationTheme.Dark ? "Dark" : "Light";
        config.Save();
    }
}
